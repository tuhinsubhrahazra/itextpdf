...
public abstract class Border {
     ...
     * @param defaultSide       the {@link Side}, that we will fallback to, if it cannot be determined by border coordinates
    ...
    protected void drawDiscontinuousBorders(PdfCanvas canvas, Rectangle boundingRectangle, float[] horizontalRadii, float[] verticalRadii, Side defaultSide, float borderWidthBefore, float borderWidthAfter) {
        ...
        Point clipPoint2;
        Point clipPoint;
        final Side borderSide = getBorderSide((float) x1, (float) y1, (float) x2, (float) y2, defaultSide);
        switch (borderSide) {
            case TOP:
